...
async function wasmFeatureDetect() {
    return {
      bigInt: async () => {
        try {
          return (await WebAssembly.instantiate(new Uint8Array([ 0, 97, 115, 109, 1, 0, 0, 0, 1, 6, 1, 96, 1, 126, 1, 126, 3, 2, 1, 0, 7, 5, 1, 1, 98, 0, 0, 10, 6, 1, 4, 0, 32, 0, 11]))).instance.exports.b(BigInt(0)) === BigInt(0);
        } catch (e) {
          return false;
        }
      },
      ...
      threads: async () => {
        try {
          return "undefined" != typeof MessageChannel && new MessageChannel().port1.postMessage(new SharedArrayBuffer(1)), WebAssembly.validate(new Uint8Array([/* ... */]));
        } catch (e) {
          return false;
        }
      },
